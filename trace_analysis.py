...
def Pyprob_IO_Kernel(batch_size, bucket_idx, rootdir,savedir=None,pickle_module=None):
    ...
    while len(trace_cache) < batch_size:
            ...
            print ('length of files:%d\ncontroled:%d\nuncontrolled:%d\nobserved:%d\nreplaced:%d\n'%(len(chosen_files),len(samples_controlled),len(samples_uncontrolled),len(samples_observed),len(samples_replaced)))
            cl=np.asarray(controlled_length)         
            print('controlled addresses lengths min:%d,max:%d,mean:%d'%(np.min(cl),np.max(cl),np.average(cl)))
            cl=np.asarray(uncontrolled_length)
            print('uncontrolled addresses lengths min:%d,max:%d,mean:%d'%(np.min(cl),np.max(cl),np.average(cl)))
            cl=np.asarray(observed_length)
            print('observed addresses lengths min:%d,max:%d,mean:%d'%(np.min(cl),np.max(cl),np.average(cl)))
            cl=np.asarray(replaced_length) 
            print('replaced addresses lengths min:%d,max:%d,mean:%d'%(np.min(cl),np.max(cl),np.average(cl)))
            print('total controlled samples:%d'%controlled_number)
            print('total uncontrolled samples:%d'%uncontrolled_number)
            print('total observed samples:%d'%observed_number)
            print('total replaced samples:%d'%replaced_number)
